...
/* One important efficiency concern is the identification of the dynamic LPUs that get activated in any
   ...
   NOTE: these function definitions and LpuIDRange data structure will be part of compiler libraries.  
*/
typedef struct {
	int startId;
	int endId;
...
/* A structure is needed to hold the ppu-ids of threads for all the spaces defined in the task. As we
   adopt a thread-per-core execution methods, these Ids are fixed and can be generated in main function
