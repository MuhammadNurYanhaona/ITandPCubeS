...
// A PThreads run function separate from the run function generated for any IT task is is needed 
// as the library requires the thread run function to confirm to a particular signature. The only
// thing this new function does in our case is that it unpack elements from the argument and call
// the actual task run functions with those parameters. 
