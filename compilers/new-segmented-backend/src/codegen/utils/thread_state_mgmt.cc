...
// partition dimensions of individual arrays. Memory for these arrays are not allocated as the copying is not 
// done for any LPU in any LPS. For memory allocation, further analysis of the compute block is needed and the 
// whole allocation logic will be handled in a separate module.
...
void generateInitializeLpusRoutine(std::ofstream &programFile, MappingNode *mappingRoot) {
	...
	// Note that we skip the root LPU as it must be set correctly for computation  to make progress and it is 
	// been correctly initialized by the setRootLpu() routine.
	...
	std::deque<MappingNode*> nodeQueue;
		...
		// otherwise, we have to call the appropriate get-LPU-count function generated before 
			...
			for (int i = 0; i < paramConfigs->NumElements(); i++) {
				...
				// if no variable is yet created for the parent LPU then create it
...
void generateComputeNextLpuRoutine(std::ofstream &programFile, MappingNode *mappingRoot) {
	...
	std::deque<MappingNode*> nodeQueue;
		...
		// call appropriate LPU construction function to populate the fields of the current LPU
