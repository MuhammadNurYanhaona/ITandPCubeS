...
class ListMetadata {
  protected:
	...
	Dimension *boundary;
	// ids of the dimensions that have been partitioned
	int *partitionedDims;
	// the total number of parts of the structure generated by the partition function. Here the count is given
	// as the number of partitions along each partitioned dimensions 
	int *partsCount;
  ...
  public:
	ListMetadata(int dimensionality, Dimension *boundary, int *partitionedDims, int *partsCount);
	inline int getDimensions() { return dimensionality; }
	inline Dimension *getBoundary() { return boundary; }
	inline int *getPartsCount() { return partsCount; }
	inline void setPadding(bool hasPadding) { this->hasPadding = hasPadding; }
	inline bool isPadded() { return hasPadding; }
	inline int *getPartitionedDims() { return partitionedDims; }
	void generateIntervalSpec(List<DataPart*> *partList);
...
class DataPartsList {
  ...
  public:
	...
	template <class type> void allocate(List<PartMetadata*> *partMetadataList) {
		...
		metadata->generateIntervalSpec(partLists[0]);
	}
