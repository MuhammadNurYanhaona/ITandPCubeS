...
int mainPartLUFV(int argc, const char *argv[]) {

	int iterations = 0;
	if (argc > 1) {
		iterations = atoi(argv[1]);
	} else {
		std::cout << "please specify how many iterations of LU factorization you want to run\n";
	...
	if (argc > 2) {
		...
		std::cout << "please specify the block size for an iteration\n";
		std::exit(EXIT_FAILURE);
	}
