...
%token <doubleConstant> Real 
%token <booleanConstant> Boolean
%token <stringConstant> Type_Name Variable_Name String

%left ','
%right '=' R_ADD R_MULT R_MAX R_MIN R_MAX_ENTRY R_MIN_ENTRY R_AVG
%left O_OR
%left O_AND
%left O_EQ O_NE
%left '<' '>' O_LTE O_GTE
...
dynamic_array	: Dimensionality T_Array Of 
		  scalar_type format				{ $$ = new ArrayType(@1, $4, $1); };
...
activation_command :						{ $$ = NULL; } 
		| Activate If Variable_Name In expr		{ $$ = new RangeExpr(new Identifier(@3, $3), $5, NULL, @1); };	 
repeat_control	: Repeat ':' From String repeat_loop 		{ $$ = new RepeatControl(new Identifier(@4, $4), $5, @1); };
repeat_loop	: For Variable_Name In expr step_expr		{ $$ = new RangeExpr(new Identifier(@2, $2), $4, $5, @1); }
		| While expr					{ $$ = $2; }
...
index_range	: names In Variable_Name restrictions		{ $$ = new IndexRangeCondition($1, new Identifier(@3, $3), $4, Join(@1, @4)); };
restrictions	:						{ $$ = NULL; }	 
		| O_AND expr					{ $$ = $2; };
...
expr		: expr '+' expr					{ $$ = new ArithmaticExpr($1, ADD, $3, @2); }
		...
		| expr R_ADD expr				{ $$ = new ReductionExpr($1, SUM, $3, @2); }
		| expr R_MULT expr				{ $$ = new ReductionExpr($1, PRODUCT, $3, @2); }
		| expr R_MAX expr				{ $$ = new ReductionExpr($1, MAX, $3, @2); }
		| expr R_MIN expr				{ $$ = new ReductionExpr($1, MIN, $3, @2); }
		| expr R_AVG expr				{ $$ = new ReductionExpr($1, AVG, $3, @2); }
		| expr R_MAX_ENTRY expr				{ $$ = new ReductionExpr($1, MAX_ENTRY, $3, @2); }
		| expr R_MIN_ENTRY expr				{ $$ = new ReductionExpr($1, MIN_ENTRY, $3, @2); }
		| constant
		| field
		| function_call
		| task_invocation
...
function_call	: Variable_Name '(' args ')'			{ $$ = new FunctionCall(new Identifier(@1, $1), $3, Join(@1, @4)); };
args		:						{ $$ = new List<Expr*>;} 
